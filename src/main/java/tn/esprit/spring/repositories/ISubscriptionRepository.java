...
public interface ISubscriptionRepository extends CrudRepository<Subscription, Long> {
    ...
    @Query("select s from Subscription s where s.typeSub = :typeS order by s.startDate asc")
    ...
    @Query("select distinct s from Subscription s where (CURRENT_TIME - s.endDate <=7) order by s.endDate")
